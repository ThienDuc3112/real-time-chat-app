...
export const getFriends = async (req: IUserRequest, res: Response) => {
  ...
  const friends = await db
    ...
    .from(friendships)
    .where(
      or(eq(friendships.user1, req.user.id), eq(friendships.user2, req.user.id))
    )
    .rightJoin(
      users,
      or(
        and(
          eq(friendships.user1, req.user.id),
          eq(users.id, friendships.user2)
        ),
        and(eq(friendships.user2, req.user.id), eq(users.id, friendships.user1))
      )
    );
  res.json(friends);
};
